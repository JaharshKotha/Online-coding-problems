...
while($t_row = pg_fetch_array($t_r))
{
	...
	if($flg==1)
	{
		$sq = "select id from filters where dbname='".$array[$i]."' and tablename='".$ro[0]."' and colname='".$t_row[0]."' and admin_uname=$u" ;
		$resu = pg_query($con2,$sq);
		if($che = pg_fetch_array($resu))
		{
	echo '<td class="'.$ro[0].'" data-attr="'.$array[$i].'">'.$t_row[0].'<input type ="checkbox" value="'.$t_row[0].'" checked="checked"><img src="img/funnel.jpg" height="10" width="10"></td>';
		}
		else{
	...
	else
	{
		...
		{
		echo '<td class="'.$ro[0].'" data-attr="'.$array[$i].'">'.$t_row[0].'<input type ="checkbox" value="'.$t_row[0].'"><img src="img/funnel.jpg" height="10" width="10"></td>';
		}
		else{
