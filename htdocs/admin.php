...
{
	echo '<tr><td>'.$row[2].'</td> <td>'.$row[3].'</td><td><input type="submit" value="remove"></td><tr>';
}
echo '</table>';
?>

<br><br><br><h4 style="color:black;"> Your existing teamates </h4><br>
<?php
$u ="'".$_SESSION['uname']."'";
$s = "select *from creat_teams where admin_uname=$u";
$r = pg_query($con,$s);
echo '<table>';
echo '<tr><th>Team Name</th>';
echo '<th>Member\'s First Name </th>';
echo '<th> Member\'s privilege  </th><tr>';
while($row = pg_fetch_array($r))
{
	echo '<tr><td style="all:unset;">'.$row[2].'</td> <td >'.$row[3].'</td><td>'.$row[4].'</td><tr>';
}
echo '</table>';
?>
