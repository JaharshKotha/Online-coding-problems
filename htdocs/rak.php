<?php
   ...
   while ($row = pg_fetch_row($r)) {
   	...
   	while ($row2 = pg_fetch_row($r1)) {
   		$associativeArray[$row2[0]] = $row2[1];
   		//    echo $row2[0];
   			//echo "\n";
   			//echo $row2[1];
   			//echo "\n";
   ...
   $associativeArrayMainD['postgres'] = $associativeArrayMain;
...
<script>
    ...
    $(document).ready(function() {
        ...
        $.each(result, function(k, v) {
            ...
            $.each(rowResult, function(k1, v1) {
                ...
                $.each(colResult, function(k2, v2) {
                    ...
                    var li1 = $(".sortColoumn li").clone();
                    var li2 = $(".beforeColoumn li").clone();
                    var li3 = $(".groupLists li").clone();

                    li1.children().attr("data-val", k1);
                    //console.log(k2);
                    li1.css('display', 'block');
                    $(".sortColoumn" + k1 + " ul").append(li1[0]['outerHTML']);


                    li2.children().attr("data-val", k1);
                    //console.log(k2);
                    li2.css('display', 'block');
                    $(".beforeColoumn" + k1 + " ul").append(li2[0]['outerHTML']);


                    li3.children().attr("data-val", k1);
                    //console.log(k2);
                    li3.css('display', 'block');
                    $(".groupLists" + k1 + " ul").append(li3[0]['outerHTML']);
        ...
        $("#rawDataList li").click(function() {
            ...
            if (($(this).text().trim() === "Raw data") || ($(this).text().trim() === "Count of rows")) {
                $("#rawDataTextField").text($(this).text());
            } else {
                $("#rawDataTextField").text($(this).text());
                $(".rawSecondLevel").show();
        ...
        $(".rawSecondLevel li").click(function() {
            var someData = $("#rawDataTextField").text();
            $("#rawDataTextField").text(someData + " " + $(this).text());
        ...
        $("#sortFieldPicker").click(function() {
            ...
            /*
              $("#sortColoumnList").toggle();
            var sortColList = $(".beforeColoumn"+$("#tableNameText").text().trim()+" section").clone();
            sortColList.find( "li" ).addClass("filterColoumnListData");
            ///$(".sortColoumnContainer").empty();
            //$(sortColList[0]['outerHTML']).insertAfter()
            $(".sortColoumnContainer").append(sortColList[0]['outerHTML']);
   
            // $("#sortColoumnList .beforeColoumn"+$("#tableNameText").text().trim()).show();
   
            $("#sortColoumnList").toggle();
            $(".sortColoumnContainer").empty();
             var result = $.parseJSON(json);
            $.each(result, function(k, v) {
            var colResult = $.parseJSON(JSON.stringify(v));
            $.each(colResult, function(k1, v1) {
            if(k1.trim()===$("#tableNameText").text().trim())
            {
            var coloumnResult = $.parseJSON(JSON.stringify(v1));
            $(".sortColoumnContainer section .text-default").text(k1.trim());
   
                 //sortCloned
            $.each(coloumnResult, function(k2, v2) {
            $(".sortColoumnContainer .List-item-title").text(k2);
            var sortLiCloned = $(".sortColoumnContainer").clone();
            alert(sortLiCloned.html());
            $(".sortColoumnContainer section ul").append(sortLiCloned[0]['outerHTML']);
   
            });
            var sortCloned = $(".sortColoumnContainer section").clone();
            sortCloned.css('display','block');
            $(".sortColoumnContainer").append(sortCloned[0]['outerHTML']);
        ...
        whereclause+= " "+$(this).find(".colQuery").text();
        whereclause+= " "+$(this).find(".midQuery").text();
        whereclause+= " "+$(this).find(".valQuery").text();
   
        firstTime=false;
         //test
        });
        var firstTime = new Boolean(true);
        $('.groupingTags:visible').each(function(i, obj) {
        if(firstTime)
        whereclause+=" group by ";
        else
        whereclause+=" , "
   
        whereclause+= " "+$(this).find(".QueryOption").text().trim();
   
        firstTime=false;
         //test
        });
        var sortInfo=""
        if($("#sortTag:visible").text().trim().length>0)
        {
        ...
        limit=" LIMIT "+$("#limit .Button--active").text();
            ...
            var query="Select * from "+$("#tableNameText").text().trim()+whereclause+sortInfo+limit;
         alert(query);
        });
        */
        $("form").submit(function() {
            ...
            $('.Query-filters:visible').each(function(i, obj) {
                ...
                whereclause += " " + $(this).find(".colQuery").text();
                whereclause += " " + $(this).find(".midQuery").attr("data-reactid");
                whereclause += " '" + $(this).find(".valQuery").text() + "'";
            ...
            $('.groupingTagsLive:visible').each(function(i, obj) {
                if (firstTime) {
                    selectClause = "";
                    whereclause += " group by " + $(this).find(".QueryOption").text().trim();
                    secselectClause = $(this).find(".QueryOption").text().trim();
            ...
            if ($("#rawDataTextField").text().trim() === "Raw data") {
                ...
                //aggData
                secselectClause += " , Count(*) as Count_of_Rows "

            }
            var sortInfo = ""
            if ($("#afterSortClick #sortTag:visible").text().trim().length > 0) {
                //alert($("#sortTag").text()+$("#sortOrder").text());
                sortInfo = " order by " + $("#sortTag").text().trim() + " " + $("#sortOrder").attr("data-attr").trim();
                // selectClause += " ,"+$("#sortTag").text().trim();
            }
            var limit = "";
            if ($("#limit .Button--active").text().trim() === "None") {} else {
                limit = " LIMIT " + $("#limit .Button--active").text();
            }
            var query = "Select " + secselectClause + selectClause + " from '" + $("#tableNameText").text().trim() +"'"+ whereclause + sortInfo + limit;
            ...
            $.ajax({
                ...
                data: {
                    queryString: query,
                    tableName: $("#tableNameText").text().trim(),
                    selectTags: selectClause + secselectClause
                },
                beforeSend: function() {},
                complete: function() {},
        ...
        <ul class="pl4 pr1 flex align-center">
            ...
            <li class="flex-align-right transition-background">
                <div class="inline-block text-white">
                    <div class="NavDropdown inline-block cursor-pointer">
                        <a class="NavDropdown-button NavItem flex align-center p2 transition-background" data-metabase-event="Navbar;Profile Dropdown;Toggle">
                            <div class="NavDropdown-button-layer">
                                <div class="flex align-center">
                                    <div style="font-size:0.85rem;border-width:1px;border-style:solid;border-radius:99px;width:2rem;height:2rem;background-color:transparent;" class="flex align-center justify-center bg-brand">FE</div>
                                    <svg name="chevrondown" fill="currentcolor" viewBox="0 0 32 32" height="8px" width="8px" class="Dropdown-chevron ml1">
                                        <path d="M1 12 L16 26 L31 12 L27 8 L16 18 L5 8 z " />
                                    </svg>
                                </div>
                            </div>
...
<div class="flex z1" id="react_qb_viz">
    <div class="wrapper full relative mb2 z1 flex flex-column">
        ...
        <div class="flex flex-full Visualization z1 px1 Visualization--errors">
...
<h3 class="List-section-title">postgres</h3>
</div>
</div>
</section>
</div>
                    ...
                    <h3 class="text-default">
                        <span   class="flex align-center">
                           <span   class="flex align-center text-slate cursor-pointer backOuter">
                              ...
                              <span   class="ml1 outerNavPointer">postgres</span>
                           </span>
                           <span  ></span>
                        </span>
                     </h3>
            ...
            <ul class="p1 border-bottom scroll-y scroll-show" style="max-height:400px;">
                <li style=" display:none;" class="List-item flex">
                    <a style="padding-top:0.25rem;padding-bottom:0.25rem;" class="flex-full flex align-center px1 cursor-pointer tableName">
                        ...
                        <h4 class="List-item-title ml2">Items</h4>
                    </a>
                </li>
            </ul>
        </section>
...
<div class="PopoverBody PopoverBody--withArrow" id="afterColoumn">
    <div style="" class="FilterPopover">
        <div style="width: 300px;">
            ...
            <div>
                <div data-reactid=".f.0.1.0" class="border-bottom p1 buttonContainer">
                    <button data-reactid=" = " class="Button Button-normal Button--medium mr1 mb1 Button--purple">Equal</button>
                    <button data-reactid=" <> " class="Button Button-normal Button--medium mr1 mb1">Not equal</button>
                    <button data-reactid=">" class="Button Button-normal Button--medium mr1 mb1">Greater than</button>
                    <button data-reactid="<" class="Button Button-normal Button--medium mr1 mb1">Less than</button>
                    <button data-reactid="BETWEEN" class="Button Button-normal Button--medium mr1 mb1">Between</button>
                    <button data-reactid=">=" class="Button Button-normal Button--medium mr1 mb1">Greater than or equal to</button>
                    <button data-reactid="<=" class="Button Button-normal Button--medium mr1 mb1">Less than or equal to</button>
                    <button data-reactid=".f.0.1.0.0:$IS_NULL" class="> '' ">Is empty</button>
                    <button data-reactid="<> ''" class="Button Button-normal Button--medium mr1 mb1">Not empty</button>
                    <button data-reactid="LIKE " class="Button Button-normal Button--medium mr1 mb1">Contains</button>
                    <button data-reactid=" NOT LIKE " class="Button Button-normal Button--medium mr1 mb1">Does not contain</button>
                    <button data-reactid=" LIKE " class="Button Button-normal Button--medium mr1 mb1">Starts with</button>
                    <button data-reactid=" LIKE " class="Button Button-normal Button--medium mr1 mb1">Ends with</button>
                </div>
                <div>
                    <ul>
...
<span class="PopoverContainer tether-element tether-element-attached-center tether-target-attached-center tether-enabled tether-element-attached-top tether-target-attached-bottom" style="top: 0px; left: 0px; position: absolute; transform: translateX(267px) translateY(225px) translateZ(0px); z-index: 3;">
      <div   class="PopoverBody PopoverBody--withArrow FilterPopover" id="rawDataList">
         <div   style="width:300px;" class="text-green" >
            <section   class="List-section List-section--open">
               <ul   class="p1 border-bottom scroll-y scroll-show" style="max-height:400px;">
                  <li   class="List-item flex List-item--selected">
                     <a   style="padding-top:0.25rem;padding-bottom:0.25rem;" class="flex-full flex align-center px1 cursor-pointer">
...
<li class="List-item flex">
    <a style="padding-top:0.25rem;padding-bottom:0.25rem;" class="flex-full flex align-center px1 cursor-pointer">
        <h4 class="List-item-title ml2">Cumulative sum of ...</h4>
    </a>
    <div class="p1"><span class="QuestionTooltipTarget"><span   class="hide"></span></span>
    </div>
</li>
